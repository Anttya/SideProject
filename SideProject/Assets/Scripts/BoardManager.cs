...
public class BoardManager : MonoBehaviour {
    ...
    void Start () {
        ...
        DrawBoard();
        SpawnPlayer();
        pInfo = player.GetComponent<PlayerInfo>(); //Reference to player's info component
    ...
    private void CalculateBoardPosition()
    {
        ...
        boardPosition = Camera.main.ScreenToWorldPoint(new Vector3((Screen.width/3)*2, 0,10));
    }
    ...
    public void CreateBoard()
    {
        ...
        for (int i = 0; i < boardDimensions.x; i++) //Iterate through columns X
        {
            for (int j = 0; j < boardDimensions.y; j++) //Iterate through rows Y
            {
                ...
                GameBoard[i, j].GetComponent<TileInfo>().index = new Vector2(i, j); //Set tile's index
